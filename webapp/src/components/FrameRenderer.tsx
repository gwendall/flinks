                                ...
                                <React.Fragment key={`button-${index}`}>
                                    {frameButton.action === "mint" ? (
                                        <MintModal
                                            ...
                                            chainId={Number(parseMintTarget(frameButton.target).chain)}
                                            {...(parseMintTarget(frameButton.target).tokenId ? {
                                                token: [
                                                    parseMintTarget(frameButton.target).contract,
                                                    parseMintTarget(frameButton.target).tokenId,
                                                ].filter(Boolean).join(':'),
                                            } : {
                                                contract: parseMintTarget(frameButton.target).contract,
                                            })}
                                            onMintComplete={(data) => {
